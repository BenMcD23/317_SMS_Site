...
const REGEN_OPTIONS: { choice: RegenChoice; label: string; title: string; blurb: string }[] = [
  {
    choice: "all",
    label: "Both messages",
    title: "Regenerate both messages?",
    blurb:
      "Replaces both messages — including any edits — with fresh AI versions of the programme data already stored on this night.",
  },
  {
    choice: "main",
    label: "Main flight message only",
    title: "Regenerate the main flight message?",
    blurb:
      "Rewrites the main flight message. The C Flight message and the uniform line are left exactly as they are.",
  },
  {
    choice: "c_flight",
    label: "C Flight message only",
    title: "Regenerate the C Flight message?",
    blurb:
      "Rewrites the C Flight message. The main flight message and the uniform line are left exactly as they are.",
  },
  {
    choice: "programme",
    label: "Re-read the programme first",
    title: "Re-read the programme and regenerate?",
    blurb:
      "Reads this night's entry from the programme doc again and rewrites both messages from it — use this if the programme has changed since this night was last generated.",
  },
];
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <Collapsible>
          <CollapsibleTrigger asChild>
            <Button variant="ghost" size="sm" className="text-muted-foreground group -ml-2 w-fit">
              <ChevronDown className="transition-transform group-data-[state=open]:rotate-180" />
              Parsed programme data
            </Button>
          </CollapsibleTrigger>
          <CollapsibleContent>
            <div className="bg-muted/40 mt-2 grid gap-4 rounded-md border p-3 text-sm sm:grid-cols-2">
              <div>
                <p className="text-muted-foreground mb-1 text-xs font-medium">Main body</p>
                <p className="whitespace-pre-wrap">{message.main_body_raw || "—"}</p>
              </div>
              <div className="flex flex-col gap-3">
                <div>
                  <p className="text-muted-foreground mb-1 text-xs font-medium">C Flight</p>
                  <p className="whitespace-pre-wrap">{message.c_flight_raw || "—"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground mb-1 text-xs font-medium">Uniform</p>
                  <p className="whitespace-pre-wrap">{message.uniform_raw || "—"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground mb-1 text-xs font-medium">DNCO</p>
                  <p>{message.dnco || "—"}</p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-muted/40 rounded-md border p-3">
          <div className="text-muted-foreground mb-1 flex flex-wrap items-center justify-between gap-x-3 gap-y-0.5 text-xs">
            <span className="font-medium">Preview</span>
            <span>
              {preview.length} chars · ~{smsSegments(preview)} SMS segment
              {smsSegments(preview) !== 1 ? "s" : ""} + greeting
            </span>
          </div>
          <p className="text-sm whitespace-pre-wrap">{preview || "Nothing to send yet."}</p>
        </div>
        ...
        {sent && (
          <div className="text-muted-foreground text-sm">
            ...
            {message.send_results &&
              ` · ${message.send_results.length - failures.length}/${message.send_results.length} delivered to Notify`}
            {failures.length > 0 && (
              <div className="text-destructive mt-2 flex flex-col gap-1">
                {failures.map((f) => (
...
export default function TextMessagesPage() {
  ...
  const upsertMessage = (updated: ParadeMessage) =>
    setMessages((prev) =>
      [...prev.filter((m) => m.id !== updated.id), updated].sort((a, b) =>
        a.parade_date.localeCompare(b.parade_date)
      )
    );
  ...
  const handleRegenerate = async (message: ParadeMessage, choice: RegenChoice): Promise<boolean> => {
    const path =
      choice === "programme" ? "/texts/generate/night" : `/texts/messages/${message.id}/regenerate`;
    const body =
      choice === "programme" ? { parade_date: message.parade_date.slice(0, 10) } : { part: choice };
  ...
  const handlePatch = async (
    ...
    try {
      ...
      replaceMessage(data);
      toast.success(
        body.status ? `Marked as ${body.status === "ready" ? "ready to send" : "draft"}.` : "Saved."
      );
      return true;
    } catch {
      toast.error("Server unreachable.");
  ...
  const handleAction = async (id: number, action: "send" | "test-send", body?: object): Promise<boolean> => {
    try {
      ...
      if (action === "send") {
        ...
        if (data.failed > 0)
          toast.warning(`Sent to ${data.sent}, but ${data.failed} failed — see card for details.`);
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 pb-16">
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Reads the {MONTHS[Number(month) - 1]} {year} programme doc and creates an AI-drafted text for
                each parade night. Existing drafts for this month are overwritten (including edits); nights
                already sent are left untouched.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      <p className="border-warning/30 bg-warning/10 text-muted-foreground flex items-start gap-2 rounded-md border px-3 py-2 text-xs">
        <Info className="text-warning mt-0.5 size-3.5 shrink-0" />
        <span>
          These drafts are AI-written and can get things wrong — read every one before you mark it ready to
          send.
        </span>
      </p>
