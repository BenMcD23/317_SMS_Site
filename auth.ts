...
function makeAdminClient() {
  const auth = new google.auth.JWT({
    ...
    key: process.env.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY?.replace(/^"|"$/g, "")?.replace(/\\n/g, "\n"),
    ...
    async jwt({ token, account, user }) {
      if (account) {
        ...
        if (account.provider === "credentials") {
          ...
          const role =
            (Object.keys(DEV_USERS) as DevRole[]).find((r) => DEV_USERS[r].email === user?.email) ?? "staff";
          return {
            ...token,
            role,
        ...
        token.expires_at = (account.id_token ? idTokenExp(account.id_token) : 0) || account.expires_at;
