...
const eslintConfig = defineConfig([
  ...
  ...nextTs,
  {
    ...
    rules: {
      // The React Compiler rule that flags setState inside an effect. Most of
      // the hits are the older "fetch on mount, then setLoading/setData"
      // pattern that predates useApiQuery; each one is a small refactor to
      // React Query, not a bug, so it warns rather than blocks CI until the
      // pages are migrated. New code should not add to the list.
